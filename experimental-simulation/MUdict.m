...
% -------------------------------------------------------------------------
%
% UPDATE (Robin):   I have slightly modified Simon's code such that the
%                   discharge times in terms of samples is also included
%                   in Disrate (first column vector).
    ...
    for j = 1:size(data1.edition.Pulsetrain,2) % Loop on each grid
        if size(data1.edition.Pulsetrain{j},1) > 0
            ...
            for k = 1:size(data1.edition.Pulsetrain{j},1) % Format the discharge times for the 1st grid
                ...
                Disrate{n,nmu1}{n} = [Distime(1:end-1)', timeDR', DRt', to', ta'];
...
clearvars -except RTfc Disratec firingprop
...
save(['MUdict_S',num2str(subject_nr),'.mat'],'Disrate','firingprop','RT','RTm')
